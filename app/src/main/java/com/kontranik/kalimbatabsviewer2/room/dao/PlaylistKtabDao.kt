...
interface PlaylistKtabDao {
    ...
    @Query("SELECT * FROM ktabs_table WHERE kTabId in (SELECT kTabId FROM PlaylistKtabCrossRef WHERE playlistId = :id)")
    fun getAllKTabsToPlaylistId(id: Long): Flow<List<KTabRoom>>

    @Query("SELECT * FROM playlist WHERE playlistId in (SELECT playlistId FROM PlaylistKtabCrossRef WHERE kTabId = :ktabId)")
    ...
    @Query("DELETE FROM playlistktabcrossref WHERE playlistId = :playlistid AND kTabId = :ktabId")
